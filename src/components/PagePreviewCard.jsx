...
export default function PagePreviewCard({ profile, links }) {
  ...
  return (
    <div className="w-72 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden bg-white dark:bg-slate-900">
      ...
      <div
        className="max-h-80 overflow-y-auto px-5 py-6 flex flex-col items-center text-center"
        style={pageStyle}
      >
        {profile.avatar_url ? (
          <img
          ...
          <div className="w-14 h-14 rounded-full mb-3" style={{ background: avatarFallbackBg }} />
        )}

        <h4 className="text-sm font-bold leading-tight" style={{ color: nameColor }}>
          {displayName}
        </h4>
        <p className="text-xs mb-2" style={{ color: handleColor }}>@{profile.username}</p>
        {profile.bio && (
          <p className="text-xs mb-4 line-clamp-2" style={{ color: bioColor }}>
            {profile.bio}
          </p>
        )}

        {shownLinks.length === 0 ? (
          <p className="text-[11px]" style={{ color: handleColor }}>No links yet.</p>
        ) : (
          <div className="w-full space-y-2">
            {shownLinks.map((link) => {
              ...
              return (
                <div
                  key={link.id}
                  className="w-full py-2 px-3 rounded-lg text-[12px] font-medium truncate"
                  style={boxStyle}
                >
                  {link.title}
                </div>
            ...
            {remaining > 0 && (
              <p className="text-[11px] pt-1" style={{ color: handleColor }}>
                +{remaining} more
              </p>
            )}
          </div>
        ...
        <p className="text-[10px] mt-5" style={{ color: footerColor }}>
          Made with Linkie
        </p>
      </div>
