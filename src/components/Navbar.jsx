...
export default function Navbar({ profile, onEditProfile, onLogoClick }) {
  ...
  // Close the mobile menu on outside click or Escape — small polish,
  // but a menu that only closes via the hamburger toggle feels stuck.
  useEffect(() => {
    if (!menuOpen) return;
  ...
  return (
    <nav className="border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 sticky top-0 z-40">
      ...
      {/* Mobile menu — a single bordered card with consistent rows,
          instead of three differently-styled elements loosely stacked. */}
      {menuOpen && (
